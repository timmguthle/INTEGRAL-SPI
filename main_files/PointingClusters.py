...
class PointingClusters: #add min time diff
    ...
    def _load_base_rsp(self):
        """
        this function loads the required base rsp for the time implied by the pointings.
        asuming that for each path the irf stays the same
        """ 
        versions = []
        for path in self._orbit_paths:
            time = extract_date_range(path)[0].replace("-", "").replace(':', '').replace("T", " ").split(".")[0][2:]
            print(time)
            version = find_response_version(time)
            if version not in versions:
                versions.append(version)
        print(f"loading irf version: {versions}")
    ...
    def _get_scw_ids(self, center_ra, center_dec, print_results=False):
        ...
        assert len(scw_ids_all) != 0, "There is no pointing for the given parameters!"
        scw_ids = []
        paths = []
        ...
        # find the needed versions of the rsp 
        ...
        for i, scw_id in enumerate(scw_ids_all):
            ...
            for path in self._orbit_paths:
                ...
                try: # investigate why this is necessary
                    ...
                    rsp = ResponseRMFGenerator.from_time(time_start, dets[0], eb, emod, self._rsp_bases[version])
                except:
