...
def identical_repeats():
    ...
    # set all the updated time to the time of the chosen pointing
    ...
    # hier wurde eigentlich ...spec_orig.fits verwendet
    with fits.open(f"{data_path}/evts_det_spec.fits") as file:
        t = Table.read(file[1])
        
    background_counts = t.copy()

    # set all the background counts to the value for the chosen pointing
    ...
    # save the new generated background + source counts to the evts_det_spec.fits
            
    hdu = fits.BinTableHDU(data=total_counts, name="SPI.-OBS.-DSP") # was bedeutet das?
