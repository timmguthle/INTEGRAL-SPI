...
def crab_broken_powerlaw(model, piv=100):
    ...
    pl.alpha.prior = Uniform_prior(lower_bound=-2.5, upper_bound=-1.4)
    pl.beta.prior = Uniform_prior(lower_bound=-3., upper_bound=-1.6)
    pl.xb = 50.
    pl.xb.free = True
    pl.xb.prior = Uniform_prior(lower_bound=20 , upper_bound=400)
...
def simulated_pl_0422(model, piv):
    ...
    pl.index.min_value = -12.5
    component1 = SpectralComponent("pl", shape=pl)
    ps = PointSource("Simulated_Source_0422", ra=ra, dec=dec, components=[component1])
